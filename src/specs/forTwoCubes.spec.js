...
let throwPersentage = {
 ...
 describe('Throwing two dice 100, 1000, 5000 times', function() {
   ...
   //   }
   //   for (let q = 2; q < 13; q++){
   //     console.log(`The number of all dropped ${q} is equal to ${quantityOfValues['quantityOf' + q]}`);
   //   }
   //
   //
   ...
   //
   //
   //   // переписать функцию для вывода проверки гипотезы на 100, 1000, 5000 бросков
   //
   // });
   ...
   //      }
   //
   //      // Consider the percentage of each amount dropped relative to the ideal (83.3 times per 1000 throws)
   //      for (let k = 2; k < Object.keys(quantityOfValues).length+2; k++ ){
   //        throwPersentage[k] = (Math.round(100 * (quantityOfValues['quantityOf' + k] / 83.3)));
   //      }
   //      console.log("\nThe Fallout Percentage of each number, relative to the ideal.\nFor 1000 rolls, 100 percent distribution would be 83.3 for each total of two dice \n" + Object.values(throwPersentage)+ "\n\n");
   //
   //      for (let j = 2; j < 13; j++){
   //        console.log(`The amount ${j} dropped out ${Object.values(quantityOfValues['quantityOf' + j])} times instead of 83.3. The percentage of deviation was ${(100 - throwPersentage[j]) + "%"}`);
   //      }
   // });
   ...
   it('Two dice rolls 5000', async function () {
     ...
     for (let q = 2; q < 13; q++){
           ...
           // Consider the percentage of each amount dropped relative to the ideal (416.6 times per 1000 throws)
           for (let k = 2; k < Object.keys(quantityOfValues).length+2; k++ ){
             throwPersentage[k] = (Math.round(100 * (quantityOfValues['quantityOf' + k] / 416.6)));
           }
           console.log("\nThe Fallout Percentage of each number, relative to the ideal.\nFor 5000 rolls, 100 percent distribution would be 416.6 for each total of two dice \n" + Object.values(throwPersentage)+ "\n\n");

           for (let j = 2; j < 13; j++){
             console.log(`The amount ${j} dropped out ${Object.values(quantityOfValues['quantityOf' + j])} times instead of 416.6. The percentage of deviation was ${(100 - throwPersentage[j]) + "%"}`);
           }
   });
 });
